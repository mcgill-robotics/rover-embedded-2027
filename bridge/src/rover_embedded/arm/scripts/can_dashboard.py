...
def api_cmd_read():
    ...
    if _commander is None:
        return jsonify({"error": "Commander not initialised"}), 503
    from rover_embedded.arm.esc_can.protocol import ReadSpec
    data = request.get_json(force=True)
    dev = int(data.get("device_id", 1))
...
def _parse_motor_type(value) -> int:
    from rover_embedded.arm.esc_can.protocol import MotorType
...
def main():
    ...
    from rover_embedded.arm.esc_can.commander import CANCommander

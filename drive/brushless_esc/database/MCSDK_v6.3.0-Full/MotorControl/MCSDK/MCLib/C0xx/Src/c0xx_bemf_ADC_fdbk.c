/**
  ...
  *          topology is used.
  * 
  *          It is specifically designed for STM32C0XX
...
/* enable ADC source trigger */
  ...
  switch (step)
  {
    ...
    case STEP_5:
      ...
      {
        LL_ADC_SetAnalogWDThresholds(ADC1, LL_ADC_AWD1, LL_ADC_AWD_THRESHOLD_HIGH, 0xFFF);
        LL_ADC_SetAnalogWDThresholds(ADC1, LL_ADC_AWD1, LL_ADC_AWD_THRESHOLD_LOW, (uint32_t) (Bemf_Threshold >> 4));   
      }
      else
      {
        LL_ADC_SetAnalogWDThresholds(ADC1, LL_ADC_AWD1, LL_ADC_AWD_THRESHOLD_LOW, 0);
        LL_ADC_SetAnalogWDThresholds(ADC1, LL_ADC_AWD1, LL_ADC_AWD_THRESHOLD_HIGH, (uint32_t) (Bemf_Threshold >> 4));   
      }
      break;
...
void BADC_SelectAdcChannel(Bemf_ADC_Handle_t *pHandle, uint8_t step)
{
  ...
  LL_ADC_SetAnalogWDMonitChannels(ADC1, LL_ADC_AWD1, __LL_ADC_ANALOGWD_CHANNEL_GROUP(__LL_ADC_DECIMAL_NB_TO_CHANNEL(pHandle->pParams_str->AdcChannel[step]),LL_ADC_GROUP_REGULAR));
}

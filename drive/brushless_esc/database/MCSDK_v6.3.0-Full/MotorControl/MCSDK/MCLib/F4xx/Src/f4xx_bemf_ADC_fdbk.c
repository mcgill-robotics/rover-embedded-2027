...
__weak void BADC_Init(Bemf_ADC_Handle_t *pHandle)
{
  ...
  if ((uint32_t)pHandle == (uint32_t)&pHandle->_Super)
  {
    ...
    */
    LL_ADC_SetChannelSamplingTime (ADCx_u, pHandle->pParams_str->AdcChannel[2], LL_ADC_SAMPLINGTIME_3CYCLES);
    LL_ADC_SetChannelSamplingTime (ADCx_v, pHandle->pParams_str->AdcChannel[1], LL_ADC_SAMPLINGTIME_3CYCLES);
    LL_ADC_SetChannelSamplingTime (ADCx_w, pHandle->pParams_str->AdcChannel[0], LL_ADC_SAMPLINGTIME_3CYCLES);
...
__weak void BADC_Start(Bemf_ADC_Handle_t *pHandle, uint8_t step, uint8_t LSModArray[])
{
  ...
  switch (step)
  {
    ...
    case STEP_5:
      ...
      {
        LL_ADC_SetAnalogWDThresholds(ADCx, LL_ADC_AWD_THRESHOLD_HIGH, 0xFFF);
        LL_ADC_SetAnalogWDThresholds(ADCx, LL_ADC_AWD_THRESHOLD_LOW, (uint32_t) (Bemf_Threshold >> 4));
      }
      else
      {
        LL_ADC_SetAnalogWDThresholds(ADCx, LL_ADC_AWD_THRESHOLD_LOW, 0);
        LL_ADC_SetAnalogWDThresholds(ADCx, LL_ADC_AWD_THRESHOLD_HIGH, (uint32_t) (Bemf_Threshold >> 4));
      }
      break;
    case STEP_2:
    case STEP_4:
...
__weak void BADC_Stop(Bemf_ADC_Handle_t *pHandle)
{
  ...
  /* Stop ADC injected conversion */
    LL_ADC_INJ_StopConversionExtTrig(ADCx_u);
    LL_ADC_INJ_StopConversionExtTrig(ADCx_v);
    LL_ADC_INJ_StopConversionExtTrig(ADCx_w);
    pHandle->ADCRegularLocked=false;    
  }
  else
  {
  ...
  if (pHandle->SpeedTimerState == COMMUTATION)
  {
    ...
    case STEP_5:
      ...
      {     
        ...
        CC_Counter = (uint32_t) (TimerSpeed_Counter + (((uint32_t)((pHandle->ZcFalling2CommDelay) * pHandle->StepTime_Up)) >> 9));
        pHandle->StepTime_Last = pHandle->StepTime_Down;
      }
      else
      {     
        pHandle->StepTime_Up =  tStepTime;
        CC_Counter = (uint32_t) (TimerSpeed_Counter + (((uint32_t)((pHandle->ZcRising2CommDelay) * pHandle->StepTime_Down)) >> 9));
...
void BADC_SelectAdcChannel(Bemf_ADC_Handle_t *pHandle, uint8_t step)
{
  ...
  LL_ADC_SetAnalogWDMonitChannels(pHandle->pParams_str->pAdc[step],

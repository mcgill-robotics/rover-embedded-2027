...
float_t MC_Perf_GetCPU_Load(const MC_Perf_Handle_t *pHandle)
{
  ...
  <#if MC.DRIVE_NUMBER != "1">
    float_t MFT_cpu_loadM2;
    float_t HFT_cpu_loadM2;
  </#if><#-- MC.DRIVE_NUMBER > 1 -->
  ...
  <#if MC.DRIVE_NUMBER != "1">
    MFT_cpu_loadM2 = (((float_t)pHandle->MC_Perf_TraceLog[MEASURE_TSK_MediumFrequencyTaskM2].DeltaTimeInCycle\
                   / (float_t)SYSCLK_FREQ ) * (float_t)MEDIUM_FREQUENCY_TASK_RATE2);
    HFT_cpu_loadM2 = (((float_t)pHandle->MC_Perf_TraceLog[MEASURE_TSK_HighFrequencyTaskM2].DeltaTimeInCycle\
                   / (float_t)SYSCLK_FREQ ) * (float_t)(PWM_FREQUENCY2/REGULATION_EXECUTION_RATE2));
  </#if><#-- MC.DRIVE_NUMBER > 1 -->

  <#if MC.DRIVE_NUMBER != "1">
    cpuLoad = MFT_cpu_loadM1 +MFT_cpu_loadM2+ HFT_cpu_loadM1+HFT_cpu_loadM2 ;
  <#else>
